...
<head>
	...
	function finish(val)
	{
		...
		if(val==0)
		{
			location.href = '<?php echo Router::url(array('controller' => 'records', 'action' => 'add', $content['Task']['id'], 0))?>/' + studySec + '/0';
			return;
		}
		...
		if(val==-1)
		{
			location.href = '<?php echo Router::url(array('action' => 'index', $this->Session->read('Iroha.theme_id')))?>';
			return;
		}
		
		// 学習履歴を残して終了の場合
		location.href = '<?php echo Router::url(array('controller' => 'records', 'action' => 'add', $content['Task']['id'], 1))?>/' + studySec + '/' + val;
		return;
	}
...
<?php
	...
	switch($content['Task']['kind'])
	{
		case 'url':
			$body = '<iframe id="contentFrame" width="100%" height="100%" scrolling="yes" src="'.h($content['Task']['url']).'"></iframe>';
			break;
		case 'movie':
			$body = '<video src="'.h($content['Task']['url']).'" controls width="100%"></video>';
			$td_style = 'vertical-align: middle;';
			break;
		case 'text':
			$body = h($content['Task']['body']);
...
<table width="100%" height="99%">
	<tr height="30">
		<td>
			<div class='content-title'><?php echo h($content['Task']['title'])?></div>
		</td>
	</tr>
	<tr>
